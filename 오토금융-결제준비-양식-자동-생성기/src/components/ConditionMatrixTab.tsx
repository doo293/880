...
// 강조 토글 뱃지 / 버튼 컴포넌트
// ============================================================================
export const HighlightToggleButton: React.FC<{
  isHighlighted?: boolean;
  onToggle: () => void;
  title?: string;
  size?: 'xs' | 'sm';
  className?: string;
}> = ({ isHighlighted, onToggle, title = '강조색 토글', size = 'xs', className = '' }) => (
  <button
    type="button"
    onClick={(e) => {
      e.stopPropagation();
    ...
    title={title}
    className={`inline-flex items-center justify-center font-bold rounded transition-all cursor-pointer select-none shrink-0 ${
      size === 'xs' ? 'px-1 py-0.5 text-[9px] gap-0.5' : 'px-1.5 py-0.5 text-[10px] gap-1'
    } ${
      isHighlighted
        ? 'bg-rose-600 hover:bg-rose-500 text-white shadow-2xs ring-1 ring-rose-400'
        : 'bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-700 border border-slate-300'
    } ${className}`}
  >
    <Sparkles className={`${size === 'xs' ? 'w-2.5 h-2.5' : 'w-3 h-3'} shrink-0`} />
    <span>강조</span>
  </button>
);

export const ModalHighlightToggle: React.FC<{
  isHighlighted?: boolean;
  onToggle: () => void;
  label?: string;
}> = ({ isHighlighted, onToggle, label = '강조' }) => (
  <button
    type="button"
    onClick={(e) => {
      e.stopPropagation();
    ...
    className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9.5px] font-bold cursor-pointer transition-all shrink-0 ${
      isHighlighted
        ? 'bg-rose-600 hover:bg-rose-500 text-white ring-1 ring-rose-400/70 shadow-xs'
...
export interface DealerConditionState {
  ...
  lumpSum: {
    nh: {
      ...
      tier1_inc: string;
      tier1_note: string;
      tier1_cust_hl?: boolean;
      tier1_inc_hl?: boolean;
      tier2_amt: string;
      tier2_cust: string;
      tier2_inc: string;
      tier2_note: string;
      tier2_cust_hl?: boolean;
      tier2_inc_hl?: boolean;
    };
    lotte: {
      amt: string;
      cust: string;
...
export const INITIAL_DEALER_CONDITION: DealerConditionState = {
  ...
  lumpSum: {
    nh: {
      ...
      tier1_cust: '1.2%',
      tier1_inc: '0.4%',
      tier1_note: '기존고객 진행가능',
      tier2_amt: '1천 이상',
      tier2_cust: '1.0%',
      tier2_inc: '0.3%',
      tier2_note: '신규는 카드 수령 후 진행',
    },
    lotte: {
      amt: '1백 이상',
      cust: '1.5%',
    ...
    hana: {
      amt: '3백 이상',
      rates: [
        { type: '', cust: '1.5%', inc: '0.4%' },
        { type: '', cust: '1.4%', inc: '0.5%' },
        { type: '', cust: '1.3%', inc: '0.6%' },
      ],
      notes: [
        '3조건 중 선택',
        '연회비 19,900원',
...
const STORAGE_KEY = 'dealer_clean_condition_matrix_v3';
...
export const ConditionMatrixTab: React.FC = () => {
  ...
  // 선수율 / 금리 개별 티어(구간) 파싱 및 업데이트 헬퍼
  const parseTierList = (dpStr?: string, rateStr?: string) => {
    const dpLines = (dpStr || '').split('\n');
    const rateLines = (rateStr || '').split('\n');
    const maxLen = Math.max(dpLines.length, rateLines.length, 1);
    const result: { dp: string; rate: string }[] = [];
  ...
  const updateDirectItemTier = (itemIdx: number, tierIdx: number, field: 'dp' | 'rate', value: string) => {
    const list = [...data.directList];
    const tiers = parseTierList(list[itemIdx].downPayment, list[itemIdx].rate);
    tiers[tierIdx][field] = value;
    const downPayment = tiers.map((t) => t.dp).join('\n');
    const rate = tiers.map((t) => t.rate).join('\n');
    list[itemIdx] = { ...list[itemIdx], downPayment, rate };
    saveToStorage({ ...data, directList: list });
  };

  const addDirectItemTier = (itemIdx: number) => {
    const list = [...data.directList];
    const tiers = parseTierList(list[itemIdx].downPayment, list[itemIdx].rate);
    tiers.push({ dp: '', rate: '' });
    const downPayment = tiers.map((t) => t.dp).join('\n');
  ...
  const removeDirectItemTier = (itemIdx: number, tierIdx: number) => {
    const list = [...data.directList];
    let tiers = parseTierList(list[itemIdx].downPayment, list[itemIdx].rate);
    if (tiers.length <= 1) {
      tiers = [{ dp: '', rate: '' }];
    } else {
      tiers = tiers.filter((_, i) => i !== tierIdx);
    }
    const downPayment = tiers.map((t) => t.dp).join('\n');
    const rate = tiers.map((t) => t.rate).join('\n');
    list[itemIdx] = { ...list[itemIdx], downPayment, rate };
  ...
  const updateAutoItemTier = (itemIdx: number, tierIdx: number, field: 'dp' | 'rate', value: string) => {
    const list = [...data.autoList];
    const tiers = parseTierList(list[itemIdx].downPaymentRate, list[itemIdx].rate);
    tiers[tierIdx][field] = value;
    const downPaymentRate = tiers.map((t) => t.dp).join('\n');
    const rate = tiers.map((t) => t.rate).join('\n');
    list[itemIdx] = { ...list[itemIdx], downPaymentRate, rate };
    saveToStorage({ ...data, autoList: list });
  };

  const addAutoItemTier = (itemIdx: number) => {
    const list = [...data.autoList];
    const tiers = parseTierList(list[itemIdx].downPaymentRate, list[itemIdx].rate);
    tiers.push({ dp: '', rate: '' });
    const downPaymentRate = tiers.map((t) => t.dp).join('\n');
  ...
  const removeAutoItemTier = (itemIdx: number, tierIdx: number) => {
    const list = [...data.autoList];
    let tiers = parseTierList(list[itemIdx].downPaymentRate, list[itemIdx].rate);
    if (tiers.length <= 1) {
      tiers = [{ dp: '', rate: '' }];
    } else {
      tiers = tiers.filter((_, i) => i !== tierIdx);
  ...
  // 개별 강조(Highlight) 토글 헬퍼 함수들
  const toggleDirectItemRateHighlight = (itemIdx: number) => {
    const list = [...data.directList];
    const current = list[itemIdx].rate_hl ?? list[itemIdx].isHighlight ?? false;
    list[itemIdx] = { ...list[itemIdx], rate_hl: !current, isHighlight: !current };
    saveToStorage({ ...data, directList: list });
  };

  const toggleDirectItemTierRateHighlight = (itemIdx: number, tierIdx: number) => {
    const list = [...data.directList];
    const tiers = parseTierList(list[itemIdx].downPayment, list[itemIdx].rate);
    const hlList = [...(list[itemIdx].tier_rate_hl || [])];
    while (hlList.length < tiers.length) {
      hlList.push(list[itemIdx].rate_hl ?? list[itemIdx].isHighlight ?? false);
    }
    hlList[tierIdx] = !hlList[tierIdx];
    list[itemIdx] = { ...list[itemIdx], tier_rate_hl: hlList };
    saveToStorage({ ...data, directList: list });
  };

  const toggleDirectItemIncHighlight = (itemIdx: number) => {
    const list = [...data.directList];
    const current = list[itemIdx].inc_hl ?? false;
    list[itemIdx] = { ...list[itemIdx], inc_hl: !current };
    saveToStorage({ ...data, directList: list });
  };

  const toggleAutoItemRateHighlight = (itemIdx: number) => {
    const list = [...data.autoList];
    const current = list[itemIdx].rate_hl ?? list[itemIdx].isHighlight ?? false;
    list[itemIdx] = { ...list[itemIdx], rate_hl: !current, isHighlight: !current };
    saveToStorage({ ...data, autoList: list });
  ...
  const toggleAutoItemTierRateHighlight = (itemIdx: number, tierIdx: number) => {
    const list = [...data.autoList];
    const tiers = parseTierList(list[itemIdx].downPaymentRate, list[itemIdx].rate);
    const hlList = [...(list[itemIdx].tier_rate_hl || [])];
    while (hlList.length < tiers.length) {
  ...
  const toggleAutoItemIncHighlight = (itemIdx: number) => {
    const list = [...data.autoList];
    const current = list[itemIdx].inc_hl ?? false;
    list[itemIdx] = { ...list[itemIdx], inc_hl: !current };
    saveToStorage({ ...data, autoList: list });
  ...
  const toggleCorporateCustHighlight = (idx: number) => {
    const list = [...data.corporate];
    list[idx] = { ...list[idx], cust_hl: !list[idx].cust_hl };
    saveToStorage({ ...data, corporate: list });
  };

  const toggleCorporateIncHighlight = (idx: number) => {
    const list = [...data.corporate];
    list[idx] = { ...list[idx], inc_hl: !list[idx].inc_hl };
    saveToStorage({ ...data, corporate: list });
  };

  const toggleNHHighlight = (field: 'tier1_cust_hl' | 'tier1_inc_hl' | 'tier2_cust_hl' | 'tier2_inc_hl') => {
    const updated = {
      ...data,
      lumpSum: {
        ...data.lumpSum,
  ...
  const toggleLotteHighlight = (field: 'cust_hl' | 'inc_hl') => {
    const updated = {
      ...data,
      lumpSum: {
        ...data.lumpSum,
        lotte: {
          ...data.lumpSum.lotte,
          [field]: !data.lumpSum.lotte[field],
        },
      },
    };
    saveToStorage(updated);
  ...
  const toggleWooriHighlight = (field: 'cust_hl' | 'inc_hl') => {
    const updated = {
      ...data,
      lumpSum: {
        ...data.lumpSum,
        woori: {
          ...data.lumpSum.woori,
          [field]: !data.lumpSum.woori[field],
        },
      },
    };
    saveToStorage(updated);
  ...
  const toggleShinhanHighlight = (field: 'cust_hl' | 'inc_hl' | 'incNew_hl' | 'incExist_hl') => {
    const updated = {
      ...data,
      lumpSum: {
        ...data.lumpSum,
        shinhan: {
          ...data.lumpSum.shinhan,
          [field]: !data.lumpSum.shinhan[field],
        },
      },
    };
    saveToStorage(updated);
  ...
  const toggleHanaRateHighlight = (rateIdx: number, field: 'cust_hl' | 'inc_hl') => {
    const rates = [...(data.lumpSum.hana.rates || [])];
    if (rates[rateIdx]) {
      rates[rateIdx] = { ...rates[rateIdx], [field]: !rates[rateIdx][field] };
      saveToStorage({
        ...data,
        lumpSum: {
          ...data.lumpSum,
  ...
  const toggleKBHighlight = (field: 'cust_hl') => {
    const updated = {
      ...data,
      lumpSum: {
        ...data.lumpSum,
  ...
  const toggleKBTierIncHighlight = (tierIdx: number) => {
    const tiers = [...(data.lumpSum.kb.tiers || [])];
    if (tiers[tierIdx]) {
      tiers[tierIdx] = { ...tiers[tierIdx], inc_hl: !tiers[tierIdx].inc_hl };
      saveToStorage({
        ...data,
        lumpSum: {
          ...data.lumpSum,
  ...
  const renderDownPaymentAndRate = (
    downPayment?: string,
    rate?: string,
    isHighlight?: boolean,
    customColor?: string,
    tierHighlights?: boolean[]
  ) => {
    const dpLines = (downPayment || '').split('\n').filter((l) => l.trim() !== '');
    const rateLines = (rate || '').split('\n').filter((l) => l.trim() !== '');

    if (dpLines.length === 0 && rateLines.length === 0) {
      return <span>-</span>;
    }
    ...
    if (dpLines.length === 0) {
      return (
        <span
          ...
          style={{
            color: isHighlight ? (customColor || styles.highlightColor) : styles.customerRateColor,
          }}
        >
          {rate}
        </span>
    ...
    if (rateLines.length === 0) {
      return <span className="text-slate-600 font-medium whitespace-pre-line">{downPayment}</span>;
    }

    const maxLen = Math.max(dpLines.length, rateLines.length);
    const rows = [];
    for (let i = 0; i < maxLen; i++) {
      const isThisTierHl = tierHighlights?.[i] !== undefined ? tierHighlights[i] : isHighlight;
      rows.push({
        dp: dpLines[i] || '',
        rt: rateLines[i] || '',
        hl: isThisTierHl,
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-0.5 leading-snug">
        {rows.map((row, rIdx) => (
          <div key={rIdx} className="flex items-center justify-center gap-2 whitespace-nowrap">
            {row.dp && <span className="text-slate-600 font-medium" style={{ fontSize: '0.92em' }}>{row.dp}</span>}
            {row.rt && (
              <span
                className="font-bold"
                style={{
                  fontSize: '1em',
                  color: row.hl ? (customColor || styles.highlightColor) : styles.customerRateColor,
                }}
              >
                {row.rt}
              </span>
                  ...
                  >
                    ...
                    <div className="col-span-2">
                      ...
                      <textarea
                        ...
                        placeholder="36~60개 월&#10;36개 월..."
                        className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1 text-white text-[11px]"
                      />
                    </div>
                    {/* 선수율 / 금리 개별 구간 목록 */}
                    <div className="col-span-5 bg-slate-900/90 p-2 rounded-xl border border-slate-700 space-y-1.5">
                      <div className="flex items-center justify-between pb-1 border-b border-slate-800">
                        <span className="text-[10px] font-bold text-slate-300">선수율 / 금리 (구간별 개별 입력)</span>
                        <button
                          type="button"
                          onClick={() => addDirectItemTier(idx)}
                          className="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-600 hover:bg-blue-500 text-white rounded text-[10px] font-bold cursor-pointer transition-colors"
                        >
                          <Plus className="w-3 h-3" />
                          <span>구간 추가</span>
                        </button>
                      ...
                      <div className="space-y-1.5 max-h-[160px] overflow-y-auto pr-0.5">
                        {parseTierList(item.downPayment, item.rate).map((tier, tIdx, allTiers) => (
                          <div key={tIdx} className="grid grid-cols-12 gap-1.5 bg-slate-950 p-1.5 rounded-lg border border-slate-800 items-center">
                            <div className="col-span-6">
                              <label className="block text-[9px] text-slate-400 mb-0.5 font-medium">선수율</label>
                              <input
                                type="text"
                                value={tier.dp}
                                onChange={(e) => updateDirectItemTier(idx, tIdx, 'dp', e.target.value)}
                                placeholder="예: 무선수, 선수 10%"
                                className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1 text-white text-[11px] font-medium"
                              />
                            </div>
                            <div className="col-span-5">
                              <label className="block text-[9px] text-emerald-400 mb-0.5 font-bold">금리</label>
                              <input
                                type="text"
                                value={tier.rate}
                                onChange={(e) => updateDirectItemTier(idx, tIdx, 'rate', e.target.value)}
                                placeholder="예: 4.3%"
                                className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1 text-emerald-300 font-bold text-[11px]"
                              />
                            </div>
                            <div className="col-span-1 flex justify-end pt-3">
                              {allTiers.length > 1 && (
                                <button
                                  type="button"
                                  onClick={() => removeDirectItemTier(idx, tIdx)}
                ...
                {data.autoList.map((item, idx) => (
                  ...
                  >
                    ...
                    <div className="col-span-5 bg-slate-900/90 p-2 rounded-xl border border-slate-700 space-y-1.5">
                      ...
                      <div className="space-y-1.5 max-h-[160px] overflow-y-auto pr-0.5">
                        {parseTierList(item.downPaymentRate, item.rate).map((tier, tIdx, allTiers) => (
                          <div key={tIdx} className="grid grid-cols-12 gap-1.5 bg-slate-950 p-1.5 rounded-lg border border-slate-800 items-center">
                            <div className="col-span-6">
                              <label className="block text-[9px] text-slate-400 mb-0.5 font-medium">선수율</label>
                              <input
                                type="text"
                                value={tier.dp}
                                onChange={(e) => updateAutoItemTier(idx, tIdx, 'dp', e.target.value)}
                                placeholder="예: 무선수, 선수 10%"
                                className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1 text-white text-[11px] font-medium"
                              />
                            </div>
                            <div className="col-span-5">
                              ...
                              <input
                                type="text"
                                value={tier.rate}
                                onChange={(e) => updateAutoItemTier(idx, tIdx, 'rate', e.target.value)}
                                placeholder="예: 5.4%"
                                className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1 text-emerald-300 font-bold text-[11px]"
                              />
                            </div>
                            <div className="col-span-1 flex justify-end pt-3">
                              {allTiers.length > 1 && (
                                <button
                                  type="button"
                                  onClick={() => removeAutoItemTier(idx, tIdx)}
                                  className="p-1 text-slate-500 hover:text-rose-400 hover:bg-slate-800 rounded cursor-pointer"
                                  title="구간 삭제"
                                >
                                  <Trash2 className="w-3 h-3" />
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-6 items-start">
              ...
              <div className="space-y-6">
                ...
                {/* [참고사항] 안내 박스 (법인캐시백 밑 여백에 배치) */}
                <div className="p-3.5 bg-amber-50/70 border border-amber-200/80 rounded-lg text-slate-700 shadow-2xs">
                  ...
                  <table className="w-full text-xs border-collapse">
                    <thead>
                      ...
                      >
                        ...
                        <th className="py-2 px-1 text-center font-bold w-[76px] whitespace-nowrap">개월수</th>
                        <th className="py-2 px-1 text-center font-bold w-[130px] whitespace-nowrap">선수율 / 금리</th>
                        <th className="py-2 px-1 text-center font-bold w-[54px] whitespace-nowrap">인센</th>
                        <th className="py-2 px-1.5 text-center font-bold whitespace-nowrap">비고</th>
                      </tr>
                    </thead>
                      ...
                      {data.directList.map((item, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {/* 개월수 */}
                            <td className="text-center py-2.5 px-1 text-slate-700 font-medium leading-snug whitespace-pre break-keep">
                              {isEditing ? (
                                <textarea
                                  rows={4}
                            ...
                            {/* 선수율 / 금리 */}
                            <td className="text-center py-2 px-1 leading-snug break-keep font-bold">
                              {isEditing ? (
                                <div className="space-y-1 py-1">
                                  {parseTierList(item.downPayment, item.rate ?? displayRate).map((tier, tIdx, allTiers) => (
                                    <div key={tIdx} className="flex items-center gap-1">
                                      <input
                                        ...
                                        placeholder="선수율"
                                        onChange={(e) => updateDirectItemTier(idx, tIdx, 'dp', e.target.value)}
                                        className="w-[62px] text-center bg-yellow-50 border border-slate-300 rounded px-1 py-0.5 text-[10.5px]"
                                      />
                                      <input
                                        type="text"
                                        value={tier.rate}
                                        placeholder="금리"
                                        onChange={(e) => updateDirectItemTier(idx, tIdx, 'rate', e.target.value)}
                                        className="w-[58px] text-center bg-yellow-50 border border-slate-300 rounded px-1 py-0.5 text-[10.5px] font-bold text-emerald-700"
                                      />
                                      {allTiers.length > 1 && (
                                        <button
                                          type="button"
                                          onClick={() => removeDirectItemTier(idx, tIdx)}
                                          className="text-slate-400 hover:text-rose-600 p-0.5 text-xs cursor-pointer"
                                          title="삭제"
                                        >
                                          ×
                                        </button>
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => addDirectItemTier(idx)}
                                    className="w-full text-center py-0.5 bg-blue-50 hover:bg-blue-100 text-blue-700 text-[9.5px] font-bold rounded border border-dashed border-blue-300 cursor-pointer"
                                  >
                                    + 구간 추가
                                  </button>
                                </div>
                              ) : (
                                renderDownPaymentAndRate(
                                  item.downPayment,
                                  displayRate,
                                  (item.brand.includes('우리') && item.type === '저금리') || item.isHighlight,
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="text-center py-2 px-1 leading-snug break-keep font-bold">
                              {isEditing ? (
                                <div className="space-y-1 py-1">
                                  {parseTierList(item.downPaymentRate, item.rate ?? displayRate).map((tier, tIdx, allTiers) => (
                                    <div key={tIdx} className="flex items-center gap-1">
                                      <input
                                        type="text"
                                        value={tier.dp}
